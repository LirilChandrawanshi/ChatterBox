...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    wsService.connect(
      ...
      (error) => {
        ...
        setConnecting(false)
        setConnectionError('Could not connect to chat server. Make sure backend is running on port 8080.')
      }
    )
  ...
  const renderMessage = (message: ChatMessage, index: number) => {
    ...
    return (
      <li key={index} className={`flex ${isOwnMessage ? 'justify-end' : 'justify-start'} mb-4 animate-fade-in`}>
        <div className={`max-w-[80%] ${isOwnMessage ? 'bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-br-sm' : 'bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white rounded-bl-sm'} rounded-2xl px-4 py-3 shadow-md`}>
          ...
          {message.type === 'CHAT' && <p className="text-sm">{message.content}</p>}
